...
pub(crate) fn parse_mix_lock(
    ...
    while !reader.take("}") {
        ...
        let name = key
            ...
            .ok_or_else(|| reader.error("invalid lock key"))?;
...
fn add_lock(
    ...
    let Term::Tuple(fields) = lock else {
        return Err(invalid());
    };
    match fields.first().and_then(Term::atom) {
        Some("hex") => {
            ...
            if repo == "hexpm" {
                ...
                let purl = format!(
                    "{}?repository_url={}",
                    package_url("generic", name, version),
                    percent_encode(repo, is_purl_byte)
                );
        ...
        Some("git") => {
            ...
            let purl = format!(
                "{}?vcs_url={}",
                package_url("generic", app, revision),
                percent_encode(url, is_purl_byte)
            );
...
impl Term<'_> {
    ...
    fn atom_list_or_nil(&self) -> bool {
        self.atom() == Some("nil")
            || matches!(self, Self::List(v) if v.iter().all(|v| v.atom().is_some()))
    }
    ...
    fn dependencies_or_nil(&self) -> bool {
        self.atom() == Some("nil")
            || matches!(self, Self::List(v) if v.iter().all(|v| matches!(v, Self::Tuple(dep) if dep.len() == 3 && dep[0].atom().is_some() && dep[1].string_or_nil() && dep[2].keyword_list())))
    }
}
...
impl<'a> Reader<'a> {
    ...
    fn value(&mut self, depth: usize) -> Result<Term<'a>, InputError> {
        ...
        self.space();
        if self.take("{") {
            return self.sequence("}", depth, false).map(Term::Tuple);
        }
        if self.take("[") {
            return self.sequence("]", depth, true).map(Term::List);
        }
        if self.take(":") {
            let atom = if self.take("\"") {
                self.quoted()?
            } else {
        ...
        if self.take("\"") {
            return self.quoted().map(Term::String);
        }
        ...
        while !self.take(end) {
            ...
            let key = if keywords {
                if self.take("\"") {
                    ...
                    self.identifier().ok().map(Cow::Borrowed)
                }
            } else {
                None
            };
        ...
        while self.offset < self.text.len() {
            ...
            if c == '"' {
                ...
                return Ok(match decoded {
                    Some(mut value) => {
                        value.push_str(&self.text[chunk..end]);
                        Cow::Owned(value)
                    }
...
mod tests {
    ...
    fn mix_lock_multiline_aliases_and_cross_file_identity() {
        ...
        assert_eq!(
            inventory
                ...
                .map(|c| c.purl.as_str())
                .collect::<BTreeSet<_>>(),
            BTreeSet::from(["pkg:hex/other@2.0.0", "pkg:hex/real_package@1.2.3"])
        );
        assert!(
            inventory
                .components
                .values()
                .all(|c| c.scope == Scope::Runtime && c.provenance.len() == 2)
        );
    }
    ...
    fn mix_lock_empty_and_escaped_literal_controls() {
        ...
        fs::write(
            dir.path().join("mix.lock"),
            r#"%{"a": {:hex, :"p\u0061ckage", "1.0.0", "quoted\"checksum", [], [], "hexpm", nil}}"#,
        )
        .unwrap();
        assert_eq!(
            scan_path(dir.path(), &config())
